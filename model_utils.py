...
def get_system_stat(stframe1, stframe2, stframe3, fps, df_fq):
    ...
    with stframe2.container():
        ...
        level, number = df_fq['Level'], df_fq['Number']
        dangerous_level = [d_level * get_dangerous_rate(d_level) for d_level in level]
        dangerous_level = sum(dangerous_level * number)


        if dangerous_level > 1:
            st.markdown("<h3 style='color:red;'>Dangerous</h3>", unsafe_allow_html=True)
        elif dangerous_level > 0.5:
            st.markdown("<h3 style='color:yellow;'>Warning</h3>", unsafe_allow_html=True)
        else:
        ...
        for num in range(0, len(number)):
            for i in range(0, number[num]):
                hist_data.append(int(level[num]))


        fig, ax = plt.subplots()
        ax.hist(hist_data, range=(1, 6))
